...
import { Search, ClipboardList, Play, PackageCheck, LifeBuoy } from "lucide-react";

const icons = [
  <Search key="discover" className="h-5 w-5" strokeWidth={1.4} />,
  <ClipboardList key="plan" className="h-5 w-5" strokeWidth={1.4} />,
  <Play key="execute" className="h-5 w-5" strokeWidth={1.4} />,
  <PackageCheck key="deliver" className="h-5 w-5" strokeWidth={1.4} />,
  <LifeBuoy key="support" className="h-5 w-5" strokeWidth={1.4} />,
];

// index -> hairline divider classes (base = mobile top divider, sm/lg override for 2-col / 5-col)
const dividerClass = [
  "",
  ...
  return (
    <section ref={sectionRef} className="border-t border-line px-6 py-28 md:px-10 md:py-36">
      <div className="mx-auto max-w-6xl">
        ...
        <div ref={gridRef} className="mt-12 grid grid-cols-1 gap-x-10 gap-y-10 sm:grid-cols-2 lg:grid-cols-5">
          {processSteps.map((step, i) => (
            <div key={step.title} className={`flex flex-col border-line ${dividerClass[i]}`}>
              <div
                data-anim="icon"
              ...
              <h3 data-anim="heading" className="mt-5 min-h-[34px] font-display text-display-md leading-tight">
                {step.title}
              </h3>
